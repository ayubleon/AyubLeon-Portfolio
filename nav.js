(function () {
  var CSS = [
    ...
    // bottom offset adds the device's own safe-area inset (0 on anything
    // without one) on top of the usual 22px, so the dock still clears the
    // home indicator on notched iPhones now that viewport-fit=cover lets
    // the page draw underneath it, instead of sitting right against it
    ...
    // tilt alternates side to side across the two peaks (-12/+12) rather
    // than both leaning the same way, so it reads as a gentle wobble
    // rather than the icon lunging in one direction twice
    "@keyframes siteNavAvatarPulse{0%,100%{transform:scale(1) rotate(0deg);}25%{transform:scale(1.22) rotate(-12deg);}50%{transform:scale(1) rotate(0deg);}75%{transform:scale(1.22) rotate(12deg);}}",
    ...
    "@media (max-width:700px){.site-nav-dock{bottom:calc(16px + env(safe-area-inset-bottom));gap:4px;}.site-nav-link{padding:11px 16px;font-size: 0.7812rem;}}",
    ...
    // same crossing-strokes animation as the case-study popup's close
    // button: transform-origin at (7,7), the icon's own center in its
    // 14x14 viewBox, and the two strokes rotate +90/-90deg on hover.
    // Since a diagonal rotated 90 degrees lands exactly on the other
    // diagonal's line, the resting and fully-hovered states both still
    // read as a plain X — the animation is the two strokes visibly
    // sweeping through and crossing each other on the way there
    ".site-contact-close-stroke{transform-origin:7px 7px;transition:transform .3s cubic-bezier(.22,1,.36,1);}",
    ".site-contact-close:hover .site-contact-close-stroke:first-child{transform:rotate(90deg);}",
    ".site-contact-close:hover .site-contact-close-stroke:last-child{transform:rotate(-90deg);}",
    ...
    ".site-contact-action{display:flex;flex-direction:column;align-items:center;gap:8px;padding:14px 8px;border-radius:16px;background:var(--al-card-hover,#2A2A2C);text-decoration:none;color:var(--al-section-title,rgba(244,238,235,0.66));transition:background .25s ease,color .25s ease;}",
  ...
  // sized to sit inline with the meta row's own 0.7812rem text rather than
  // the 24px viewBox default used above — those are standalone tappable
  // icons in their own grid cell; these two are just small glyphs leading
  // into a line of text, so they're scaled down and vertically nudged to
  // sit on the text baseline instead of the ~24px default which would
  // tower over the line
  var MAP_PIN_SVG = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex:0 0 auto;vertical-align:-1px;"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>';
  var CLOCK_SVG = '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex:0 0 auto;vertical-align:-1px;"><circle cx="12" cy="12" r="10"/><path d="M12 6v6h4"/></svg>';
  var CLOSE_SVG = '<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">' +
    '<path class="site-contact-close-stroke" d="M13 1L1 13" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>' +
    '<path class="site-contact-close-stroke" d="M1 1L13 13" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>' +
  '</svg>';
  ...
  function navHTML(links) {
    ...
    return (
      ...
      '<div class="site-contact-card" data-contact-card role="dialog" aria-modal="true" aria-label="Contact Ayub Leon">' +
        ...
        '<p class="site-contact-meta" data-contact-meta>' + MAP_PIN_SVG + ' Nairobi, Kenya &nbsp;&nbsp; ' + CLOCK_SVG + ' UTC+3 &middot; <span data-contact-clock></span></p>' +
